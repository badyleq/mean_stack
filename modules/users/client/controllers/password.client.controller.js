(function () {
  ...
  function PasswordController($scope, $stateParams, UsersService, $location, Authentication, PasswordValidator, Notification) {
    ...
    function onRequestPasswordResetSuccess(response) {
      ...
      vm.credentials = null;
      Notification.success({ message: response.message, title: '<i class="glyphicon glyphicon-ok"></i> Email ze zmianą hasła został wysłany' });
    }
    ...
    function onResetPasswordSuccess(response) {
      ...
      Notification.success({ message: '<i class="glyphicon glyphicon-ok"></i> Zmiana hasła zakończona sukcesem' });
      // And redirect to the index page
      $location.path('/password/reset/success');
    }

    function onResetPasswordError(response) {
      Notification.error({ message: response.data.message, title: '<i class="glyphicon glyphicon-remove"></i> Zmiana hasła zakończona niepowodzeniem', delay: 4000 });
    }
  }
}());
